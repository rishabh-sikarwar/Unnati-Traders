    ...
    <div className="min-h-screen bg-gray-100 p-6 md:p-8 pt-24">
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <div className="flex justify-between items-center print:hidden">
          <Link href="/billing" className="text-[#522874] font-bold flex items-center gap-2 hover:underline">
            <ArrowLeft className="w-4 h-4" /> Back to Billing
          </Link>
          {/* Add a client component or vanilla JS onclick to print */}
          <button onClick="window.print()" className="bg-[#522874] text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-[#3d1d56]">
        ...
        <div className="bg-white p-10 rounded-sm shadow-md print:shadow-none print:p-0">
          ...
          <div className="flex justify-between items-start border-b-2 border-gray-800 pb-6 mb-6">
            <div>
              <h1 className="text-3xl font-black text-[#522874] tracking-tighter">UNNATI TRADERS</h1>
              <p className="text-gray-600 mt-1">{invoice.location?.address || "Gwalior, MP"}</p>
              <p className="text-gray-600">Issued under Rule 46 of CGST Rules</p>
            </div>
            <div className="text-right">
              <h2 className="text-2xl font-bold text-gray-800">TAX INVOICE</h2>
              <p className="font-bold text-gray-600 mt-2">Invoice No: {invoice.invoiceNumber}</p>
            ...
            <tbody>
              {invoice.items.map((item, index) => (
                <tr key={item.id} className="border-b border-gray-200 text-sm">
                  ...
                  <td className="py-3 px-2 font-medium">{item.product.modelName} ({item.product.size})</td>
                  <td className="py-3 px-2 text-center">{item.quantity}</td>
                  <td className="py-3 px-2 text-right">₹{item.unitPrice.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                  <td className="py-3 px-2 text-right font-bold">₹{item.totalPrice.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex justify-end">
            <div className="w-1/2 space-y-2 text-sm">
              <div className="flex justify-between border-b border-gray-200 pb-2">
                ...
                <span className="font-bold">₹{invoice.subtotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
              </div>
              <div className="flex justify-between border-b border-gray-200 pb-2">
                <span className="text-gray-600">Total Tax (CGST + SGST)</span>
                <span className="font-bold">₹{invoice.totalGst.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
              </div>
              <div className="flex justify-between bg-gray-100 p-2 border-y-2 border-gray-800 mt-2">
                <span className="font-bold text-lg">Grand Total</span>
                <span className="font-black text-lg">₹{invoice.grandTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
              </div>
            </div>
          </div>
