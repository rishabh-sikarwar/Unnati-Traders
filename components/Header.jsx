      ...
      >
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 md:h-[70px]">
            ...
            <nav
              className={`hidden ${isInternalUser && !isRoleLoading ? "xl:flex" : "lg:flex"} items-center gap-1`}
            >
                  ...
                  <nav className="hidden lg:flex items-center gap-0.5 bg-black/25 backdrop-blur-sm px-1.5 py-1.5 rounded-xl border border-white/10 shadow-inner animate-in fade-in duration-300">
                    {staffLinks.map((item) => (
                      ...
                      >
                        <item.icon
                          size={14}
                          className={`${item.color} opacity-90`}
                        />
                        {item.name}
                      </Link>
                    ))}
                  ...
                  {staffLinks.map((item) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-1.5 rounded-md ${isActive(item.href) ? "bg-white/15" : "bg-[#522874]/10"}`}
                        >
                          <item.icon
                            size={15}
                            className={
