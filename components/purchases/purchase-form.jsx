...
export default function PurchaseForm({ products, locations, userId }) {
  ...
  // If user adds a tyre in another tab and switches back, this fetches the new data instantly!
  useEffect(() => {
  ...
  const addItem = () =>
    setCart([{ rowId: Math.random().toString(36).substr(2, 9), productId: "", quantity: 1, unitCost: "" }, ...cart]);
  ...
  // DUPLICATE PREVENTION: Array of currently selected product IDs
  const selectedProductIds = useMemo(() => cart.map(item => item.productId).filter(Boolean), [cart]);
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedItems = cart.map((item, index) => {
      if (!item.productId) throw new Error(`Please select a tyre for item #${index + 1}`);
      return {
        ...
        <div className="bg-white p-5 md:p-6 rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              ...
              <input
                required value={supplierName} onChange={(e) => setSupplierName(e.target.value)}
        ...
        <div className="bg-white p-5 md:p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex justify-between items-center mb-5 border-b border-gray-100 pb-4">
            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
              <PackageOpen className="w-5 h-5 text-[#522874]" /> Received Materials
            </h2>
            <button
          ...
          <div className="space-y-4">
            {cart.length === 0 && (
              <div className="text-center py-10 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 font-medium bg-gray-50">
                Click <span className="text-[#522874] font-bold">"Add Item"</span> to record incoming stock.
              </div>
            )}
            ...
            {cart.map((item) => {
              ...
              // Only show products that aren't already selected in another row
              const availableProducts = products.filter(p => !selectedProductIds.includes(p.id) || p.id === item.productId);

              return (
                <div key={item.rowId} className="flex flex-col gap-3 p-4 bg-gray-50 rounded-xl border border-gray-200 shadow-sm relative animate-in fade-in slide-in-from-top-4 duration-300">
                  <div className="w-full relative">
                    ...
                    <SmartTyreSelector
                      ...
                      onSelect={(val) => updateItem(item.rowId, "productId", val)}
                    />
                  </div>
                  <div className="flex flex-wrap sm:flex-nowrap items-end gap-3">
                    ...
                    <div className="flex-1 sm:flex-none sm:w-32">
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Buying Cost</label>
                      <input
                        required type="number" min="1" value={item.unitCost}
                        onChange={(e) => updateItem(item.rowId, "unitCost", e.target.value)}
                        className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#522874] outline-none h-[42px]"
                        placeholder="₹0.00"
                      />
                    </div>
        ...
        <div className="bg-[#3d1d56] text-white p-6 md:p-8 rounded-xl shadow-lg lg:sticky lg:top-28">
          ...
          <div className="border-t border-white/20 pt-2 mb-6">
            <div className="flex flex-col items-end gap-1 mt-4">
              <span className="text-sm font-bold text-white/80 uppercase">Total Purchase Value</span>
